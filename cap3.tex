...
dados do CALIFA. Este ``kit de ferramentas'' foi chamado de PyCASSO ({\em Python
CALIFA} \starlight {\em Synthesis Organizer}). PyCASSO foi desenvolvido em Pyton
e é, a grosso modo, composto de três partes.
...
CALIFA. O artigo que apresenta o DR2, de \citet{GarciaBenito2015}, também
utiliza as massas da mesma forma (ver Figura \ref{fig:DRMass}). O mesmo artigo
inclui a medição da PSF do CALIFA (ver Figura \ref{fig:DR2PSF}), apresentado
neste trabalho na Seção \ref{sec:psf:medida}.

\citet{GonzalezDelgado2014b} explora a relação massa--metalicidade estelar em
300 galáxias do CALIFA, utilizando PyCASSO. Em \citep{GonzalezDelgado2015} se
utiliza, além de PyCASSO, a decomposição bojo--disco (ver Seção
...
compõem uma zona, enquanto as intensivas são uma propriedade comum à todos os
...
extensivas passam por um processo apelidado de ``dezonificação'', descrito na
Seção \ref{sec:pycasso:Pycasso}.
...
$R_{50}$. As galáxias da amostra são separadas em caixas com $15$ galáxias cada,
segundo a sua massa. O crescimento em massa em função do tempo é calculado para
cada pixel de cada galáxia e somado em cada caixa de massa.
...
caixas, têm um crescimento gradual mais ou menos constante em todos os
raios. As galáxias mais massivas, a parte superior das caixas, apresentam um
...
representam cortes horizontais na caixa de mais baixa massa, no
...
\begin{figure}
	...
	massa estelar contendo 15 galáxias cada. (cima) Todas as galáxias. As caixas em
	massa estão codificados em cores. (meio) Galáxias dominadas por disco. (baixo)
	Galáxias esferoidais ($C \geq 2,8$). Retirado de \cite[figura
